...
int findBestMarkNumber(int* marks, int size) {
	...
	if (size % DAY_NUMBER == 6 || size < DAY_NUMBER) {
		return - 1;
	}
	...
	int last_mark = 0;
	int first_day = 0;
	int last_day = DAY_NUMBER;
	while (last_day <= size) {
		for (int i = first_day; i < last_day && i < size; i++)
		{
			if (*(marks + i) == 5) {
				expected++;
			}
		}
		if (expected == 7 || (expected == 2 && last_day == size)) {
			last_mark = expected;
			break;
		}
		else if (expected > last_mark) {
			last_mark = expected;
		}
		first_day++;
		last_day++;
